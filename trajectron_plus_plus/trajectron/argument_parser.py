...
parser.add_argument("--prefetch_factor",
                    help="batches to prefetch per worker when num_workers>0; use 1 to reduce memory",
                    type=int,
                    default=2)

parser.add_argument("--amp",
                    help="enable automatic mixed precision (AMP) training (may cause NaN with VAE-style loss)",
                    action='store_true',
                    default=False)
...
parser.add_argument('--eval_kde',
                    help='whether to compute KDE during eval, options are yes and no',
                    type=str,
                    default='yes')
