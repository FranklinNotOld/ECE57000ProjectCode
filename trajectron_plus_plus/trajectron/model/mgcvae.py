...
class MultimodalGenerativeCVAE(object):
    ...
    def create_node_models(self):
        ...
        #   Cone Encoder (B-B)  #
        ##########################
        cone_cfg = self.hyperparams.get('cone_encoder')
        if cone_cfg and cone_cfg.get('enabled', False):
            ...
            self.add_submodule(self.node_type + '/cone_encoder',
                               model_if_absent=ConePointNetEncoder(
                                   input_dim=cone_cfg.get('input_dim', 2),
                                   hidden_dim=cone_cfg.get('hidden_dim', 64),
                                   output_dim=cone_cfg.get('output_dim', 32)))
        ...
        if cone_cfg and cone_cfg.get('enabled', False):
            #              Cone Encoder (Mode B-B)
            x_size += cone_cfg.get('output_dim', 32)
    ...
    def obtain_encoded_tensors(self,
        ...
        ################
        if self.hyperparams['use_map_encoding'] and self.node_type in self.hyperparams['map_encoder']:
            if self.log_writer and self.hyperparams.get('log_mgcvae_viz', True) and (self.curr_iter + 1) % 500 == 0:
                map_clone = map.clone()
        ...
        # Cone encoding (Mode B-B)
        cone_cfg = self.hyperparams.get('cone_encoder')
        if cone_cfg and cone_cfg.get('enabled', False) and cones is not None:
            cone_rel, cone_mask = cones
            encoded_cones = self.node_modules[self.node_type + '/cone_encoder'](cone_rel, cone_mask)
            x_concat_list.append(encoded_cones)
    ...
    def project_to_GMM_params(self, tensor) -> (torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor):
        ...
        log_sigmas = torch.clamp(log_sigmas, min=-10., max=10.)
    ...
    def encoder(self, mode, x, y_e, num_samples=None):
        ...
        if mode == ModeKeys.TRAIN:
            _latent_lw = self.log_writer if self.hyperparams.get('log_latent_viz', True) else None
            kl_obj = self.latent.kl_q_p(_latent_lw, '%s' % str(self.node_type), self.curr_iter)
        else:
            kl_obj = None
    ...
    def decoder(self, mode, x, x_nr_t, y, y_r, n_s_t0, z, labels, prediction_horizon, num_samples):
        ...
        if self.hyperparams['log_histograms'] and self.log_writer is not None and self.hyperparams.get('log_mgcvae_viz', True):
    ...
    def train_loss(self,
        ...
        :param map: Tensor of Map information. [bs, channels, x, y]
        :param prediction_horizon: Number of prediction timesteps.
        :param cones: Optional tuple of (cone_rel [B,K,2], mask [B,K]) for Mode B-B.
