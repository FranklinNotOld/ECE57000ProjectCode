...
class GeometricMap(Map):
    ...
    """
    _map_chunk_size = 64  # Set to int to enable sub-batch map cropping (reduces peak GPU memory)
    ...
    def __init__(self, data, homography, description=None):
        ...
        self._last_padded_device = None
        self._torch_map = None
        self._torch_map_device = None

    def clear_cuda_cache(self):
        """Drop cached CUDA tensors so the object can be safely pickled for DataLoader workers."""
        self._torch_map = None
        self._torch_map_device = None
        self._last_padded_map = None
        self._last_padded_device = None
    ...
    def torch_map(self, device):
        ...
        if self._torch_map is not None and self._torch_map_device == dev:
            return self._torch_map
        self._torch_map = torch.tensor(self.data, dtype=torch.uint8, device=dev)
        self._torch_map_device = dev
        return self._torch_map
    ...
    def get_padded_map(self, padding_x, padding_y, device):
        dev = torch.device(device) if isinstance(device, str) else device
        if (self._last_padding == (padding_x, padding_y) and
                self._last_padded_device == dev):
        ...
        self._last_padded_map = torch.full((self.data.shape[0],
                                           ...
                                           False, dtype=torch.uint8, device=dev)
    ...
    @classmethod
    def _get_cropped_maps_impl(cls, maps, scene_pts, patch_size, rotation, device):
        """Core single-batch map crop implementation (original logic)."""
        ...
        centers_list = [s_map.to_map_points(scene_pts[np.newaxis, i]) for i, s_map in enumerate(maps)]
        centers = torch.tensor(np.array(centers_list), dtype=torch.long, device=device).squeeze(dim=1) \
        ...
        if rotation is not None:
            if isinstance(rotation, torch.Tensor):
                angles = rotation.detach().clone().to(device=device)
            else:
                angles = torch.tensor(rotation, dtype=torch.float32, device=device)
        else:
    ...
    def get_cropped_maps_from_scene_map_batch(cls, maps, scene_pts, patch_size, rotation=None, device='cpu'):
        ...
        """
        chunk_size = cls._map_chunk_size
        batch_size = scene_pts.shape[0]
        if chunk_size is not None and batch_size > chunk_size:
            chunks = []
            for start in range(0, batch_size, chunk_size):
                end = min(start + chunk_size, batch_size)
                chunk_rot = rotation[start:end] if rotation is not None else None
                chunks.append(cls._get_cropped_maps_impl(
                    maps[start:end], scene_pts[start:end], patch_size, chunk_rot, device))
            return torch.cat(chunks, dim=0)
        return cls._get_cropped_maps_impl(maps, scene_pts, patch_size, rotation, device)
