...
try:
    import ruranges.numpy as ruranges_numpy
    _RURANGES_AVAILABLE = True
except ImportError:
    _RURANGES_AVAILABLE = False


class _IntervalTree:
    """
    NCLS-compatible interval tree for overlap queries in MultiNode.
    Uses ruranges when available (fast), otherwise falls back to pure Python.
    """
    ...
    def find_overlap(self, start, end):
        """Yield (start, end, id) for each interval overlapping [start, end)."""
        if len(self.starts) == 0:
            return
        if _RURANGES_AVAILABLE:
            idx1, _ = ruranges_numpy.overlaps(
                starts=self.starts.astype(np.int32),
                ends=self.ends.astype(np.int32),
                starts2=np.array([start], dtype=np.int32),
                ends2=np.array([end], dtype=np.int32),
            )
            for i in idx1:
                yield (int(self.starts[i]), int(self.ends[i]), int(self.ids[i]))
        else:
            # Pure Python fallback: intervals overlap [start, end) if
            # interval_start < end AND interval_end > start
            for i in range(len(self.starts)):
                if self.starts[i] < end and self.ends[i] > start:
                    yield (int(self.starts[i]), int(self.ends[i]), int(self.ids[i]))
...
class MultiNode(Node):
    def __init__(self, node_type, node_id, nodes_list, is_robot=False):
        ...
        self.interval_tree = _IntervalTree(starts, ends, ids)
